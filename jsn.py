...
def split_takeout_json():
    ...
    Output needs to be manually surrounded with '[]' to ensure it's valid JSON.
    Segments are filtered to only include related activities.
    """
    for month in next(walk('takeout'), (None, None, []))[2]:
        with open('json/' + month.split('_')[0] + '.json', 'a') as g:
            g.write('[')
            for obj in loads(j.read())['timelineObjects']:
                if 'activitySegment' in obj:
                    if obj['activitySegment']['activityType'] in ['SKIING', 'SNOWBOARDING', 'IN_GONDOLA_LIFT']:
                        g.write(dumps(obj['activitySegment']) + ',\n')
            g.write(']')
        with open('json/' + month, 'r') as j:
            for obj in loads(j.read()):
                with open('jsn/' + obj['duration']['endTimestamp'].split('T')[0] + '.json', 'a') as k:
                    k.write(dumps(obj) + ',')
            ...
            for segment in loads(jsn.read()):
                if 'simplifiedRawPath' in segment:
                    ...
                    for c, t in zip(res.json()['results'], segment['simplifiedRawPath']['points']):
                        ...
                        gpx.write(f'\t\t\t\t<time>{t["timestamp"]}</time>\n')
                        gpx.write(f'\t\t\t</trkpt>\n')
